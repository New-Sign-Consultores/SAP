...
using RestSharp;
using SVDTERECEP.Common.CRUD;
using SVDTERECEP.Common.NEWSIGN;
using System;
...
namespace SVDTERECEP.Common
{
    public static class DTENewSign
    {
        static CultureInfo cultureInfo = new CultureInfo("es-CL");
        //static string apiUrl = "http://172.16.0.13/api/Core.svc/core";
        //static string apiKey = "2ec64694-829f-4a6e-b369-dc8936320e09";
        //static string query;// = "(FchEmis:2023-08-08)";
        //static string Environment = "P";
        public static void GETListDTE()
        {
            ConexionSAP();
            getConfig();
            string query = null;
            DateTime FchDesde, FchHasta;
            string Fdesde= null, Fhasta = null;
            Fdesde = Properties.Settings.Default.FDESDE;
            Fhasta = Properties.Settings.Default.FHASTA;
            if (string.IsNullOrEmpty(Fdesde) && string.IsNullOrEmpty(Fhasta))
            {
                FchDesde = new DateTime(DateTime.Now.AddDays(-5).Year, DateTime.Now.AddDays(-5).Month, DateTime.Now.AddDays(-5).Day);
                FchHasta = DateTime.Now;
            }
            else
            {
                FchDesde = Convert.ToDateTime(Fdesde);
                FchHasta = Convert.ToDateTime(Fhasta);
            }
            string TipoDoc = "Todos";

            string Ffecha = null, FTipo = null, RUTRecep = null;
            RUTRecep = DTECompany.RUTREC;// "78058870-5";
            ...
            try
            {
                ...
                Ffecha = " FchEmis:[" + FchDesde.ToString("yyyy-MM-dd") + " TO " + FchHasta.ToString("yyyy-MM-dd") + "]";
                query = RUTRecep + Ffecha + FTipo;
                string QueryBase64 = Base64Encode(query);
                string Objeto = "/PaginatedSearch/" + DTECompany.ENVIRONMENT + "/R/" + QueryBase64 + "/1/500";
                var restClient = new RestClient();
                var restRequest = new RestRequest(DTECompany.URLDTELIST + Objeto, Method.GET);
                ...
                if (restResponse.StatusDescription.Equals("OK"))
                {
                    ...
                    CantDocSAP = common.DocInTable(FchDesde.ToString("yyyy-MM-dd"), FchHasta.ToString("yyyy-MM-dd"), "'33','34','56','61'");
                                ...
                                else
                                {
                                    ...
                                    if (documentXML.TieneArchivo)
                                    {
                                        ...
                                        if (resultDTE.Success)
                                        {
                                            ...
                                            aSRDTE.Code = documentXML.RUTEmisor + documentXML.TipoDTE + documentXML.Folio;
        ...
        public static void ConexionSAP()
        {
            SAPbobsCOM.Company oCompany = new SAPbobsCOM.Company();
            string cDbServerType = Properties.Settings.Default.DbServerType;
            SAPbobsCOM.BoDataServerTypes DbServerType = (SAPbobsCOM.BoDataServerTypes)0;
            switch (cDbServerType)
            {
                case "MS2008":
            ...
            oCompany.CompanyDB = Properties.Settings.Default.CompanyDB;
            oCompany.Server = Properties.Settings.Default.Server;
            oCompany.language = SAPbobsCOM.BoSuppLangs.ln_Spanish_La;
            oCompany.UserName = Properties.Settings.Default.UserName;
            oCompany.Password = Properties.Settings.Default.Password;
            int conec = oCompany.Connect();
            int iError;
            string sError;
            if (conec != 0)
            {

                oCompany.GetLastError(out iError, out sError);
            }
            else
            {
                GC.KeepAlive(Conex.oCompany);
                Conex.oCompany = oCompany;
            }

        }
        public static bool ValidarDTE(string RUT, string Tipo, string Folio)
        {
            bool existe = false;
            CRUD_ASRDTE cRUD_ASRDTE = new CRUD_ASRDTE();
        ...
        public static ResultDTE GETXMLDTE(DocumentXML documentXML)
        {
            ResultDTE resultDTE = new ResultDTE();
            try
            {
                ...
                var restClient = new RestClient();
                var restRequest = new RestRequest(DTECompany.URLDTELIST + Objeto, Method.POST);
                restRequest.AddHeader("AuthKey", DTECompany.KEY);
                restRequest.AddHeader("Content-Type", "application/json");
                var body = @"{                                                  " + "\n" +
                @"	""Environment"":""" + DTECompany.ENVIRONMENT + @""",                    " + "\n" +
                @"	""Group"":""R"",                                            " + "\n" +
        ...
        public static void GETPDFDTE(DocumentXML documentXML)
        {
            try
            {
                string Objeto = "/RecoverPDF_V2";
                #region Filtros 
                string Filter = "/" + DTECompany.ENVIRONMENT + "/R" + documentXML.RUTEmisor + "/" + documentXML.TipoDTE + "/" + documentXML.ID;
                ...
                if (restResponse.StatusDescription.Equals("OK"))
                {
                    ...
                    if (recoverXML_V2XML.Result == "0")
                    {
                        //ResultDTE resultDTE = ObtenerDTE(recoverXML_V2XML.Data);
                    }
                    else
                    {
                        //error
...
#pragma warning restore CS0168 // La variable 'ex' se ha declarado pero nunca se usa
        ...
        private static RecoverXML_V2XML GetRecoverXML_V2(string RecoverXML_V2String)
        {
        ...
        public static ResultDTE ObtenerDTE(String decodeStringXml)
        {
            ResultDTE resultDTE = new ResultDTE();
        ...
        private static SearchResultXML GetSearchResultXML(string searchResultString)
        {
