...
namespace SVDTERECEP.Common
{
    class Common
    {
        ...
        public ResultRefSAP GetRefOC(string foliosOC, string RutEmisor)
        {
            ...
            try
            {
                ...
                query = @" SELECT SUM(T0.DocTotal) AS DocTotal, 
                                Convert(VARCHAR(100),STUFF((
                                    ...
                                    FROM OPOR T0 WHERE CONVERT(VARCHAR(100), T0.DocNum) IN(" + foliosOC + @")  AND T0.LicTradNum = '" + RutEmisor + @"'
                                    AND T0.CANCELED <> 'Y'
                                        FOR XML PATH('')

                                    ),1,1, '')) AS DocNum
                                    FROM OPOR T0
                                    WHERE CONVERT(VARCHAR(100), T0.DocNum) IN(" + foliosOC + @")  AND T0.LicTradNum = '" + RutEmisor + @"'
                                    AND T0.DocStatus != 'C'
                                    AND T0.CANCELED <> 'Y'
                                    GROUP BY T0.DocType ";
        ...
        public ResultRefSAP GetRefEM(string foliosEM, string RutEmisor)
        {
            ResultRefSAP resultRefSAP = new ResultRefSAP();
            try
            {
                ...
                query = @" SELECT SUM(T0.DocTotal) AS DocTotal, 
                                Convert(VARCHAR(100),STUFF((
                                    SELECT ','+ CONVERT(VARCHAR(30),T0.DocNum)
                                    FROM OPDN T0 WHERE CONVERT(VARCHAR(100), T0.DocNum) IN(" + foliosEM + @")  AND T0.LicTradNum = '" + RutEmisor + @"'
                                    AND T0.CANCELED <> 'Y'
                                        FOR XML PATH('')

                                    ),1,1, '')) AS DocNum
                                    FROM OPDN T0
                                    WHERE CONVERT(VARCHAR(100), T0.DocNum) IN(" + foliosEM + @")  AND T0.LicTradNum = '" + RutEmisor + @"'
                                    AND T0.DocStatus != 'C'
                                    AND T0.CANCELED <> 'Y'
                                    GROUP BY T0.DocType ";
                SAPbobsCOM.Recordset recordset = (SAPbobsCOM.Recordset)(Conex.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset));
